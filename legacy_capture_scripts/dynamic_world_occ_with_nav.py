...
VOICE_EVENT_STALE_SEC = 3.0

_last_voice_command: Optional[str] = None
    ...
    try:
        while True:
            ...
            if tcp and tcp.has_odometry_data():
                ...
                # POSM 報回的 X 軸在實際前進時會減少，因此取負號讓世界 +Y 代表往前。
                world_x = od_pose.y_m
                world_y = -od_pose.x_m
            ...
            cv2.imshow("Fast world occupancy", cv2.resize(world_fast, (GRID_W * 4, GRID_H * 4), interpolation=cv2.INTER_NEAREST))
            ...
            status_lines.append((f"detection: {det_status}", det_color))
            voice_color = (0, 255, 0) if not voice_events else (0, 165, 255)
            status_lines.append((f"voice queue: {len(voice_events)}", voice_color))
            if goal_rc:
                gx, gy = goal_rc
            ...
            cv2.imwrite("dynamic_world_occ_with_nav.png", display)
