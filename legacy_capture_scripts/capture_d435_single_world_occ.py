...
from realsense_planner.odometry import OdometryClient
...
MIRROR_X = False        # 若左右顛倒，可設 True 將 ROI 佔據圖左右翻轉
...
def _integrate_roi_into_world(occ_roi: np.ndarray, pose_xy_theta, world_img: np.ndarray) -> None:
    ...
    # 將世界坐標轉換為世界地圖的像素座標
    center_world = (
        (roi_center_world_x - WORLD_X[0]) / CELL_M,
        (WORLD_Y[1] - roi_center_world_y) / CELL_M,
    )
    ...
    print("offset=",offset_col,offset_row)
...
def main():
    ...
    if pose_xytheta is None:
        ...
        print("[odom] pose before transform:", pose_xytheta)
        odom_x, odom_y, odom_yaw =pose_xytheta
        # 里程計座標系轉換到世界座標系：
        ...
        world_x = odom_y
        world_y = odom_x
        world_yaw = odom_yaw + YAW_OFFSET_DEG
        pose_xytheta = (world_x, world_y, world_yaw)
        print("[odom] pose after  transform:", pose_xytheta)
    _integrate_roi_into_world(occ_roi, pose_xytheta, world)

    world_rgb = cv2.cvtColor(world, cv2.COLOR_GRAY2BGR)
    # draw using world (x_m, y_m) coordinates so the fan aligns with world map
    sensor_world_xy = (pose_xytheta[0], pose_xytheta[1])
    draw_backward_fan_reference(
        world_rgb,
        sensor_world_xy,
        robot_yaw_deg=pose_xytheta[2],
        x_res=CELL_M,
        y_res=CELL_M,
    ...
    # keep a pixel/grid coordinate for drawing the robot marker
    sensor_rc = (
