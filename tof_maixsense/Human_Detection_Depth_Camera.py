...
class MaixSenseController:
    ...
    def _write_position_file(self, horizontal_distance: float, person_distance: float,
        ...
        """
        嘗試寫入 human_position.txt。若 /tmp 無法寫入，就退回到其它可寫目錄。
        允許使用 HUMAN_POSITION_FILE 指定完整路徑。
        """
        env_override = os.environ.get("HUMAN_POSITION_FILE")
        candidates: List[Path] = []
        if env_override:
            candidates.append(Path(env_override).expanduser())
        candidates.append(Path(tempfile.gettempdir()) / "human_position.txt")
        candidates.append(Path(__file__).resolve().parent / "human_position.txt")
        candidates.append(Path.cwd() / "human_position.txt")
        ...
        for path in candidates:
            try:
                ...
                with path.open("w", encoding="utf-8") as file_obj:
                    file_obj.write(f"{horizontal_distance:.1f},{is_too_left},{is_too_right},{distance_status}\n")
                    file_obj.write(f"DIAGONAL:{person_distance:.1f}\n")
                    file_obj.write(f"VERTICAL_OFFSET:{self.vertical_offset_mm:.1f}\n")
                    file_obj.write("XXX\n")
                return
        ...
        if not self._position_file_warning_emitted:
            detail = "; ".join(f"{p}: {e}" for p, e in errors) if errors else "無法尋找可寫目錄"
            print(f"⚠️ 無法寫入 human_position.txt，請使用 HUMAN_POSITION_FILE 指定路徑或提供可寫目錄 ({detail})",
                  file=sys.stderr)
            self._position_file_warning_emitted = True
    ...
    def find_person(self, distance_image: np.ndarray) -> Optional[Tuple[int, int, int, int, int, int, float, float]]:
        """
        ...
        Returns:
            如果找到物體，返回 (x, y, w, h, cx, cy, median_distance, contour_area)
            否則返回 None
        """
        try:
            # --- 1. 調整感興趣的距離範圍 ---
            ...
            for contour in contours:
                ...
                if area < 500: # 假設人的最小面積，可以嘗試 100, 200, 500 等值
                    continue
            ...
            return (x, y, w, h, cx, cy, person_distance, contour_area)
