...
SENSOR_WIDTH_PX = 100
FOV_DEGREES = 60.0
DEG_PER_PIXEL = FOV_DEGREES / SENSOR_WIDTH_PX
SENSOR_CENTER_PX = SENSOR_WIDTH_PX / 2.0
...
class MaixSenseController:
    ...
    def __init__(
        ...
        vertical_offset_mm: float = 530.0,
        ...
        self.vertical_offset_mm = vertical_offset_mm
    ...
    def _handle_detections(self, detections: Optional[List[PersonDetection]]) -> None:
        ...
        if detections:
            best = detections[0]
            distance_mm = float(best.median_distance)
            horizontal_distance_mm = self._compute_horizontal_distance(distance_mm)
            angle_deg = self._compute_angle_deg(best.cx)
    ...
    def _compute_horizontal_distance(self, slant_distance_mm: float) -> float:
        """
        Convert the MaixSense-reported diagonal distance into a horizontal ground-plane distance.
        """

        if slant_distance_mm <= self.vertical_offset_mm:
            return 0.0
        return math.sqrt(slant_distance_mm**2 - self.vertical_offset_mm**2)

    def _compute_angle_deg(self, centroid_x_px: float) -> float:
        """Convert detection centroid (pixels) into a horizontal angle relative to the sensor center."""

        angle_offset_px = float(centroid_x_px) - SENSOR_CENTER_PX
        return angle_offset_px * DEG_PER_PIXEL
