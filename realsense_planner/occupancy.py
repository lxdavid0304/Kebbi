...
def draw_backward_fan_reference(
    ...
    sensor_pos: Optional[Tuple[float, float]],
    *,
    ...
    # When passing world coordinates (meters) provide ROI origin so we can map to image pixels.
    roi_x_min: Optional[float] = None,
    roi_y_max: Optional[float] = None,
) -> np.ndarray:
    """
    Draw a backward-facing fan overlay.

    `sensor_pos` may be either a grid/image coordinate `(row, col)` (ints) or a
    world coordinate `(x_m, y_m)` (floats). When world coordinates are provided,
    `x_res`/`y_res` are used together with `roi_x_min` and `roi_y_max` (or
    `cfg.ROI` if those are not given) to map to image pixels.
    
    `robot_yaw_deg`: Robot heading in degrees (0=forward/+Y, 90=left/+X, -90=right/-X).
                     The fan will be drawn in the backward direction relative to this heading.
    """
    if (
        sensor_pos is None
    ...
    # Determine whether sensor_pos is already an image (row,col) or world (x,y).
    is_int_rc = False
    try:
        is_int_rc = isinstance(sensor_pos[0], (int, np.integer)) and isinstance(sensor_pos[1], (int, np.integer))
    except Exception:
        is_int_rc = False

    if is_int_rc:
        cy, cx = int(sensor_pos[0]), int(sensor_pos[1])
    else:
        # sensor_pos is world (x_m, y_m) — map to image (row,col)
        x_m, y_m = float(sensor_pos[0]), float(sensor_pos[1])
        if roi_x_min is None or roi_y_max is None:
            try:
                roi = globals().get("ROI", None) or cfg.ROI
                roi_x_min = float(roi["x_min"]) if roi_x_min is None else roi_x_min
                roi_y_max = float(roi["y_max"]) if roi_y_max is None else roi_y_max
            except Exception:
                # Cannot determine ROI mapping — abort
                return img_color

        col = int(round((x_m - float(roi_x_min)) / float(x_res)))
        row = int(round((float(roi_y_max) - y_m) / float(y_res)))
        cy, cx = row, col
    ...
    # Calculate fan center: backward (180°) + robot heading
    fan_center_deg = 180.0 + robot_yaw_deg
    start_deg = fan_center_deg - fan_deg / 2.0
    end_deg = fan_center_deg + fan_deg / 2.0
